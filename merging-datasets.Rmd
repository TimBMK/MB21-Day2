...
```{r}
data_vac <- read_csv("/home/rstudio/ECPR/MB21-Day1/tmp/COVID-19_Vaccinations_in_the_United_States_County.csv.tar.gz")
...
data_vac <- data_vac %>% mutate(Date = mdy(Date))
```
...
```{r}
data_demo %>% distinct(FIPS) %>% nrow()

data_demo %>% anti_join(data_vac , by = "FIPS")

data_demo %>% distinct(FIPS) %>% anti_join(data_pres %>% distinct(FIPS), by = "FIPS")

data_demo %>% anti_join(data_pres, by = "FIPS")
...
```{r}
data_vac_latest <- data_vac %>% slice_max(Date)
```
...
```{r}
dbWriteTable(conn = db, name = "demo", value = data_demo)
dbWriteTable(conn = db, name = "trump", value = df_trump)
dbWriteTable(conn = db, name = "vac", value = data_vac)
